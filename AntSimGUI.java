...
public class AntSimGUI extends JFrame
{
	...
	/**
	 *	set the current simulation speed
	 *
	 *	@param	speed		String indicating simulation delay speed in milliseconds
	 */
	 ...
	 *	simulation time and the simulation delay speed
	 */
	private class ControlPanel extends JPanel
	{
		...
		// button for displaying Ant Colony running stats
		private JButton displayStatsButton;
		
		// button for slowing the run mode
		private JButton runSlowerButton;
		
		// button for speeding up the run mode
		private JButton runFasterButton;
		...
		// label for displaying the current speed delay
		...
		private void initComponents()
		{
			...
			displayStatsButton = new JButton("Display Statistics");
			displayStatsButton.addActionListener(buttonHandler);
			displayStatsButton.setToolTipText("Display running statistics for the ant colony");

			// label for displaying simulation speed factor
			speedLabel = new JLabel();
			speedLabel.setFont(new Font("Verdana", Font.BOLD, 12));
			
			// decrease speed button
			runSlowerButton = new JButton("Decrease Speed");
			runSlowerButton.addActionListener(buttonHandler);
			runSlowerButton.setToolTipText("Click to decrease the running speed of the simulation");
			
			// increase speed Button
			runFasterButton = new JButton("Increase Speed");
			runFasterButton.addActionListener(buttonHandler);
			runFasterButton.setToolTipText("Click to increase the running speed of the simulation");
		...
		private void layoutComponents()
		{
			...
			this.add(displayStatsButton);
			this.add(runSlowerButton);
			this.add(speedLabel);		
			this.add(runFasterButton);
			this.add(runButton);
			this.add(stepButton);
			this.add(timeLabel);
		}
		...
		/**
	 	 *	set the current simulation speed delay
	 	 *
	 	 *	@param	time		String indicating simulation speed delay in terms of milliseconds
	 	 */
		public void setSpeed(String speed)
		{
		...
		private class ButtonHandler implements ActionListener
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				else if (b.getText().equals("Display Statistics"))
				{
					// display current statistics for the any colony
					fireSimulationEvent(SimulationEvent.DISPLAY_STATISTICS_EVENT);
				}				
				else if (b.getText().equals("Decrease Speed"))
				{
					// fire to decrease the running speed
					fireSimulationEvent(SimulationEvent.DECREASE_SPEED_EVENT);
				}
				else if (b.getText().equals("Increase Speed"))
				{
					// fire to increase the running speed
					fireSimulationEvent(SimulationEvent.INCREASE_SPEED_EVENT);
				}
